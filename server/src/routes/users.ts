...
router.get('/', auth, isAdmin, asyncHandler(async (req: Request, res: Response) => {
...
router.get('/:id', auth, isAdmin, asyncHandler(async (req: Request, res: Response) => {
...
router.patch('/:id', auth, isAdmin, asyncHandler(async (req: Request, res: Response) => {
...
router.get('/:id/orders', auth, isAdmin, asyncHandler(async (req: Request, res: Response) => {
  const pag = getPagination(req, 20, 50);
  const { count } = await supabase
    .from('orders')
    .select('*', { count: 'exact', head: true })
