    ...
    >
      ...
      <div className="flex items-start gap-3 relative z-10">
        <div className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 ${isReview ? 'bg-[rgba(255,191,0,0.08)]' : 'bg-[rgba(255,191,0,0.05)]'}`}>
          {isReview ? (
            <Sparkles className={`w-4 h-4 ${isActive ? 'text-[#FFBF00]' : 'text-[#FFBF00]'}`} />
          ) : (
            <UserPlus className={`w-4 h-4 ${isActive ? 'text-[#FFBF00]' : 'text-[#FFBF00]'}`} />
          )}
        </div>
        <div className="flex-1 min-w-0">
          <div className={`flex items-center gap-2 ${!isActive ? 'opacity-60' : ''}`}>
            <span className={`text-xs font-semibold truncate ${isActive ? 'text-[#FFBF00]' : 'text-[#FFBF00]'}`}>
