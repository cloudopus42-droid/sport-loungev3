...
const FALLBACK_FLAVORS: Flavor[] = [
  ...
  { id: '16', name: 'Lounge Premium', category: 'Авторские', emoji: '💎', color: '#B08D57', is_active: true },
];
...
export function BookingPage() {
  ...
  return (
    <div className="relative min-h-screen bg-dark-bg text-white overflow-hidden">
      {/* Ambient glows */}
      <div className="fixed top-0 right-0 w-[600px] h-[600px] bg-[#B08D57] opacity-[0.03] blur-[150px] rounded-full pointer-events-none z-0" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 relative z-10">
          ...
          <div className="sticky top-0 h-[50vh] lg:h-screen flex flex-col items-center justify-center">
            ...
            <div className="absolute top-1/4 right-4 lg:right-12 z-20 flex flex-col gap-2">
              {flavorIcons.map((fi, i) => (
                <motion.div
                  ...
                  exit={{ opacity: 0, x: 20 }}
                  className="w-9 h-9 rounded-full flex items-center justify-center text-sm shadow-[0_0_16px_rgba(176,141,87,0.1)]"
                  style={{ background: `rgba(176,141,87,0.08)`, border: '0.5px solid rgba(176,141,87,0.2)' }}
                >
                  {fi.emoji}
                </motion.div>
              ))}
            ...
            <div className="w-full h-full relative">
              ...
              <div className="absolute bottom-8 left-[15%] right-[15%] h-px bg-[rgba(176,141,87,0.12)]">
                <motion.div
                  className="h-full bg-gradient-to-r from-[#B08D57] via-[#C4A46B] to-[#B08D57]"
        ...
        <div className="lg:col-span-5 overflow-y-auto lg:h-screen p-4 lg:p-8 space-y-5 pb-24 lg:pb-8">
          ...
          <div className="text-center lg:text-left">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-[#B08D57]/20 bg-[#B08D57]/5 text-[9px] font-semibold text-[#B08D57] uppercase tracking-[0.2em] mb-3">
            ...
            <h1 className="text-2xl lg:text-3xl font-semibold tracking-tight mb-1 font-heading">
              Собери свой <span className="text-[#B08D57]">кальян</span>
            </h1>
          ...
          <div className="liquid-glass rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-3">
              <Flame className="w-3.5 h-3.5 text-[#B08D57]" />
                ...
                <motion.button
                  ...
                  className={`relative flex flex-col items-center gap-1.5 p-2.5 rounded-xl transition-all duration-300 ${
                    bowlIndex === bowl.index
                      ? 'bg-[rgba(176,141,87,0.08)] border border-[rgba(176,141,87,0.3)] shadow-[0_0_16px_rgba(176,141,87,0.05)]'
                      : 'bg-[rgba(255,255,255,0.02)] border border-transparent hover:border-[rgba(176,141,87,0.15)]'
                  }`}
                >
                  <div
                  ...
                  <span className={`text-[8px] font-medium text-center leading-tight ${bowlIndex === bowl.index ? 'text-[#B08D57]' : 'text-white/40'}`}>
                    {bowl.name}
                  </span>
                  {bowlIndex === bowl.index && (
              ...
              {FLAVOR_CATS.map(cat => (
                <button key={cat} onClick={() => setFlavorCategory(cat)}
                  className={`px-2.5 py-1 rounded-full text-[8px] font-semibold whitespace-nowrap transition-all border ${
                    flavorCategory === cat
                      ? 'bg-[#B08D57] text-[#0b0807] border-[#B08D57]/30'
                      : 'text-white/40 bg-white/5 border-transparent hover:border-[#B08D57]/20'
                  }`}>{cat}</button>
              ))}
            </div>
                  ...
                  return (
                    <motion.button key={flavor.name} whileTap={{ scale: 0.96 }}
                      ...
                      className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[10px] transition-all border ${
                        sel
                          ? 'bg-[rgba(176,141,87,0.08)] border-[rgba(176,141,87,0.25)] text-[#B08D57]'
                          : 'bg-white/[0.02] border-transparent text-white/40 hover:border-[rgba(176,141,87,0.12)]'
                      }`}
                    >
                  ...
                  className={`flex-1 py-2 rounded-xl text-[10px] font-semibold transition-all border ${
                    strength === s
                      ? s === 'light' ? 'bg-[rgba(76,175,80,0.1)] border-[rgba(76,175,80,0.3)] text-[#4CAF50]'
                        : s === 'medium' ? 'bg-[rgba(176,141,87,0.1)] border-[rgba(176,141,87,0.3)] text-[#B08D57]'
          ...
          <div className={`liquid-glass rounded-2xl p-4 border ${aiMood.length > 0 ? 'border-[rgba(176,141,87,0.2)]' : ''}`}>
            <div className="flex items-center gap-2 mb-3">
              <Bot className="w-3.5 h-3.5 text-[#B08D57]" />
              <h3 className="text-[10px] uppercase tracking-[0.15em] font-semibold text-[#B08D57]">ИИ-Миксолог</h3>
            </div>
                ...
                return (
                  <button key={mood.id} onClick={() => setAiMood(prev => prev.includes(mood.id) ? prev.filter(id => id !== mood.id) : [...prev, mood.id])}
                    className={`px-2.5 py-1.5 rounded-lg text-[9px] transition-all border ${
                      sel ? 'bg-[rgba(176,141,87,0.1)] border-[rgba(176,141,87,0.3)] text-[#B08D57]' : 'bg-white/[0.02] border-transparent text-white/40 hover:border-[rgba(176,141,87,0.12)]'
                    }`}
                  >
            ...
            {aiRecommendations.length > 0 && (
              <div className="p-3 rounded-xl bg-[rgba(176,141,87,0.03)] border border-[rgba(176,141,87,0.1)]">
                <p className="text-[9px] text-[#B08D57] font-semibold mb-2 flex items-center gap-1">
                    ...
                    <button key={flavor} onClick={() => toggleFlavor(flavor)}
                      className={`px-2 py-1 rounded-lg text-[8px] border transition-all ${
                        selectedFlavors.includes(flavor)
                          ? 'bg-[rgba(176,141,87,0.1)] border-[rgba(176,141,87,0.25)] text-[#B08D57]'
                          : 'bg-white/[0.03] border-white/5 text-white/50 hover:border-[rgba(176,141,87,0.15)]'
                      }`}
                    >
                      {flavor}
                    </button>
          ...
          {/* Price & Order */}
          <div className="liquid-glass rounded-2xl p-5 border border-[rgba(176,141,87,0.12)] bg-gradient-to-br from-[rgba(176,141,87,0.03)] to-transparent">
            <div className="flex items-center justify-between mb-4">
                  ...
                  {PRICES.map(p => (
                    <button key={p} onClick={() => setPrice(p)}
                      className={`px-3 py-1 rounded-lg text-xs font-semibold transition-all border ${
                        price === p
                          ? 'bg-[rgba(176,141,87,0.1)] border-[rgba(176,141,87,0.3)] text-[#B08D57]'
                          : 'bg-white/[0.02] border-transparent text-white/30 hover:border-white/10'
                      }`}
                    >{p} ₽</button>
                  ))}
              ...
              <div className="text-right">
                ...
                <p className="text-2xl font-bold font-heading text-[#B08D57]">{price} ₽</p>
              </div>
            </div>
              <motion.button
              ...
              className="w-full py-3.5 rounded-xl bg-gradient-to-r from-[#B08D57] to-[#C4A46B] text-black text-xs font-bold uppercase tracking-[0.12em] shadow-[0_4px_20px_rgba(176,141,87,0.25),0_0_40px_rgba(176,141,87,0.1)] hover:shadow-[0_4px_28px_rgba(176,141,87,0.35),0_0_50px_rgba(176,141,87,0.15)] transition-all duration-300 flex items-center justify-center gap-2"
            >
          ...
          {activeOrder && (
            <motion.div
              ...
              className="liquid-glass rounded-2xl p-4 border border-[rgba(176,141,87,0.08)]"
            >
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs font-mono text-[#B08D57]">{timeText}</span>
                  <ChevronRight className={`w-3 h-3 text-white/30 transition-transform ${showOrderTracker ? 'rotate-90' : ''}`} />
                </div>
              </button>
              {showOrderTracker && (
                <div className="space-y-3 pl-3">
                  <div className="absolute left-[18px] top-8 bottom-4 w-px bg-gradient-to-b from-[#B08D57] to-white/5" />
                    ...
                    return (
                      <div key={stage.id} className={`flex items-start gap-3 relative ${isCompleted || isActive ? 'opacity-100' : 'opacity-20'}`}>
                        <div className={`w-6 h-6 rounded-full border flex items-center justify-center z-10 ${
                          isActive ? 'border-[#B08D57] text-[#B08D57]' : isCompleted ? 'border-[#B08D57] text-[#B08D57]' : 'border-white/10 text-white/20'
                        }`}>
                          {isCompleted ? <Check className="w-2.5 h-2.5 stroke-[3]" /> : <div className={`w-1 h-1 rounded-full ${isActive ? 'bg-[#B08D57]' : 'bg-transparent'}`} />}
                        </div>
                        <div className="pt-0.5">
                          <h5 className={`text-[10px] uppercase tracking-wider font-bold ${isActive ? 'text-[#B08D57]' : 'text-white'}`}>{stage.label}</h5>
                  ...
                  <button onClick={handleCallMaster} disabled={masterCalled}
                    className="mt-3 w-full py-2.5 rounded-xl border border-[rgba(176,141,87,0.2)] bg-[rgba(176,141,87,0.04)] hover:bg-[rgba(176,141,87,0.1)] disabled:bg-white/5 disabled:border-white/10 text-[9px] font-bold text-[#B08D57] disabled:text-white/20 uppercase tracking-[0.12em] transition-all"
                  >
        ...
        {showConfirmModal && selectedMix && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            <motion.div
              ...
              className="w-full max-w-md bg-[#0D0F13] border border-[rgba(176,141,87,0.15)] rounded-[16px] p-6 shadow-[0_24px_64px_rgba(0,0,0,0.55)] relative overflow-hidden"
            >
              <div className="absolute top-0 left-0 w-full h-px bg-gradient-to-r from-transparent via-[#B08D57] to-transparent" />
              ...
              <form onSubmit={handleSubmit(onOrderSubmit)} className="space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  <textarea {...register('specialNotes')} placeholder="Покислее, полегче..."
                    className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-xs text-white h-16 resize-none focus:border-[#B08D57] focus:outline-none transition-colors placeholder:text-white/20"
                  />
                </div>
                <div className="flex gap-3 pt-2">
                  ...
                  <button type="submit" disabled={loading}
                    className="flex-1 py-3 rounded-xl bg-gradient-to-r from-[#B08D57] to-[#C4A46B] text-black text-[9px] font-bold uppercase tracking-wider shadow-[0_4px_20px_rgba(176,141,87,0.25)] transition-all disabled:opacity-50"
